...
const definition = {
    ...
    exposes: [e.switch().withEndpoint('l1'), e.switch().withEndpoint('l2'), e.power(), e.current(), e.voltage(), e.energy(), e.power_on_behavior(['off', 'on', 'previous']).withEndpoint('l1'), e.power_on_behavior(['off', 'on', 'previous']).withEndpoint('l2')],
    endpoint: (device) => {
        return {'l1': 1, 'l2': 2};
    },
    ...
    configure: async (device, coordinatorEndpoint, logger) => {
        ...
        await reporting.currentSummDelivered(endpoint11, {min: 10, change:[0, 360000], max: 600});
